...
_client: InternalBoardClient | None = None


def client() -> InternalBoardClient:
    global _client
    if _client is None:
        st = load_settings()
        _client = InternalBoardClient(
...
def _workspace_schema() -> dict:
    """Committed first-party board schema. Missing/invalid schema is a hard error: field
...
def _coerce_field_value(database: str, field: str, value: str):
    if value == "":
        raise ValueError(
            "empty writes are not supported by the board action contract; set a "
            "new value or use a purpose-built clear operation")
    kind, options = _field_type_options(_field_spec(database, field))
    if kind == "select":
...
def set_item_field(database: str, title: str, field: str, value: str) -> str:
    ...
    requires a purpose-built clear operation."""
...
def remove_item_field_value(database: str, title: str, field: str, value: str) -> str:
    ...
    if not kept:
        return (f"removing {needle!r} would clear {field!r}; blank clears are "
                "not supported until a governed clear operation is implemented")
    new_value = ", ".join(kept)
...
def network_health() -> dict:
    """Liveness of every hop in the agent request network: local board store,
    ...
    probe("board_store", lambda: client().list_row_ids("todos"))
