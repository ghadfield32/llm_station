...
**Notes:** "Add skillopt to Kanban and your normal agents so skills can get better: https://www.linkedin.com/posts/shubhamsaboo_wait-whatyou-can-now-train-agent-skills-share-7485916464504389634-7qzY/" **Research verdict (2026-07-23): TRIAL — strong adopt candidate.** It is **microsoft/SkillOpt** (MIT, ~14.8k★, `pip install skillopt`, v0.2.0 2026-07-02): treats a skill markdown file as trainable parameters for a frozen model — rollout → reflect → **validation-gated edit acceptance** (our champion-challenger gate, in text space) → deployable best_skill.md. Harness-NATIVE to Claude Code + Codex CLIs (our exact executors; +19.1 avg pts inside the Claude Code harness) and `openai_compatible` backend points at our LiteLLM gateway. Directly attacks the harness-score Skills 0/17 gap. Costs: custom scoreable env from our task history; rollouts consume subscription quota per epoch. First step: sandboxed venv → SearchQA in direct-chat mode vs LiteLLM → then a 10-20-task env from our harness-score Skills checks, one epoch, best_skill.md vs no-skill onto the KPI leaderboard. Details: [RESEARCH_NOTES_2026-07.md](reference/RESEARCH_NOTES_2026-07.md). **ADOPTED as a challenger — WIRING_VERIFIED (2026-07-24):** installed v0.2.0 on Windows; wired `openai_compatible → local Ollama qwen3:8b` (no egress/key); materialized SearchQA (400/200/1400); real no-skill baseline `selection hard=0.6667`; full loop E2E `EXIT=0` with the accept-only-if-improves **gate correctly rejecting non-improving edits** (`accept=0 reject=2`); found+fixed 2 Windows blockers (`PYTHONUTF8=1`; `train_size` must match split). NOT a lift claim (8B optimizer proves the loop, not capability). Full packet + KPI leaderboard: [capability-assessment/skillopt/](../../evaluation/capability-assessment/skillopt/README.md). Next (operator-gated): capable-optimizer PILOT run on ≥2 scenarios → then Tier-2 custom llm_station-task env via `claude_code_exec`/`codex_exec`.
...
#### AGT-19 · Policy-enforcement enablement blockers (before AGENT_SESSION_POLICIES_ENABLED)
`⛔ BLOCKED` · **Target:** _TBD_ · **Done:** _—_
**Repo:** `llm_station`
**Priority:** P1
**Source:** fresh-Sol deep security review 2026-07-24 (AGT-14 pre-enablement gate) — verdict **DO-NOT-ENABLE**
**Notes:** The declarative session-policy stack (AGT-14 p1 on main + p2) MUST NOT have its flag flipped until these are fixed. Four blockers, all found by the independent deep review: **(1) CRITICAL — enforcement is observational not preventative**: the tool-call hook evaluates on `tool_started`/`command_started` which arrive AFTER the harness began; on ASK it records a pending approval and returns (no adapter pause/cancel — adapters record approval `effective: False`), so ASK is an effective auto-pass and DENY can't guarantee the action didn't run. Needs causal enforcement (pause the harness BEFORE the action, bind resume to the approval). **(2) mandatory server floor**: policies are opt-in via a spec's `policy_refs`; an unspec'd session gets ALLOW (`resolve([])→ALLOW`), so `level: server` is only ordering, not a floor. Needs an always-applied server-level set. **(3) cost fails open silently**: `cost_budget` returns ALLOW when cost is None (no usage service / `AGENT_WORKER_USAGE` off) with no degradation event — a selected cost policy silently disappears. Needs fail-closed-or-warn. **(4) human-verified ASK resolution**: the generated-approval proxy checks only the bearer token, not a verified human/operator allowlist. **What HELD (deep review):** board ALLOW/ASK never bypasses the human-token apply wall; `resolve()` monotone; closed handler enum (no import injection); no permission-profile mutation; flag-off identity (after the finding-4 fix). Evidence: [agt-14 RUNDOC §9](../projects/agt-14-declarative-policies/RUNDOC.md). Gate: AGT-19 clears → fresh-Sol re-review → operator enables the flag.
...
**Notes:** Check out the agents CLI for the kanban and ensure we use it for the metrics that help us better answer. **IDENTIFIED by operator (2026-07-23): google/agents-cli** — Google's Apache-2.0 CLI + skill suite (5.3k★, v1.2.0 released 2026-07-23) that teaches coding agents (Claude Code/Codex listed as supported) to build/evaluate/deploy ADK agents. **The metrics-relevant piece for our kanban:** `agents-cli eval generate` (run agent on eval dataset → traces) + `agents-cli eval grade` (LLM-as-judge with adaptive rubrics) and the `google-agents-cli-eval` skill — that trace→grade separation with rubric-based judging maps directly onto our AGT-12 task-completion KPI loop. **Honest limits:** the scaffold/deploy/publish surface is ADK + Google-Cloud-centric (we don't deploy there; cloud routes stay operator-gated), and local eval assumes Gemini via AI Studio key. **Verdict: TRIAL (narrow)** — install the skills in a scratch context, study the eval skill's methodology, and mine the rubric/judge structure for AGT-12; skip scaffold/deploy/publish. Secondary candidate for raw session metrics stays AgentsView (kenn-io/agentsview, MIT). Details: [RESEARCH_NOTES_2026-07.md](reference/RESEARCH_NOTES_2026-07.md). **Packet 1 IMPLEMENTED 2026-07-24** (Sol gpt-5.6-sol xhigh, isolated worktree): ported the methodology (NOT the cloud tool) as a local, OFF-by-default, `supporting_evidence_only` **adaptive-rubric LLM-as-judge** — typed `Rubric`/`RubricCriterion` contracts, a PURE scorer (`improvement/rubric.py`: median-across-samples + weighted-overall, reuses jury.py κ math, NEVER imputes a missing score → overall `None`+flagged), `sampling_count` self-consistency, loss-pattern analyzer, and an injected-executor `rubric_judge` runner (never auto-launches a model). Registered in `framework-evals.yaml` (enabled:false, local Ollama `local-judge`). Machine-proven contracts: `is_decision_gate() is False`, not in any promotion path, no fabricated scores, no provider route (validate forbidden-providers PASS). Host-verified: `cc validate` PASS, 44 tests pass (rubric+safety+jury+framework), ruff + new-module mypy clean. Fable review APPROVE. Run-doc: [docs/projects/agt-1-rubric-eval/RUNDOC.md](../projects/agt-1-rubric-eval/RUNDOC.md). Feeds AGT-12's KPI leaderboard; complements AGT-17 (SkillOpt).
...
**Notes:** https://www.linkedin.com/posts/ole-lehmann-918a6094_lol-someone-already-open-sourced-claude-design-share-7482938344658587648-Ivzr/ **Research verdict (2026-07-23): SKIP as dependency** — the project is **Open Design** (nexu-io/open-design, Apache-2.0, ~81k★): a design-generation product driven by your existing coding agent, NOT a Claude-style component library; license clean. **ADOPTED (operator-directed, same day): the DESIGN.md convention** — [services/agent_kanban_ui/DESIGN.md](../../services/agent_kanban_ui/DESIGN.md) now defines the cockpit's design contract (tokens from styles.css, layout invariants, chat-surface rules for runtime-agnostic chrome across Claude/Codex/future agents, agent workflow rules). Every agent UI packet must read + conform to it; chat polish rides KAN-4/KAN-8 under this contract. Details: [RESEARCH_NOTES_2026-07.md](reference/RESEARCH_NOTES_2026-07.md).
...
**Notes:** Typed, `extra="forbid"` policy documents for agent sessions at three stricter-first levels (session → agent → server) returning ALLOW/DENY/ASK; ASK maps onto our existing approval-wall idiom. First builtins: ask-on-os-tools, max-tool-calls-per-session, cost-budget with soft ask-thresholds (wired to the usage layer's budget concept, PR #34 workstream). Policies may only tighten the hard floors (human-only resolutions, read-only cockpit, destructive double-agreement) — never loosen. Feeds KAN-17. Run-doc: [docs/projects/agt-14-declarative-policies/RUNDOC.md](../projects/agt-14-declarative-policies/RUNDOC.md). **Packet 1 IMPLEMENTED 2026-07-24** (Sol gpt-5.6-sol xhigh, isolated worktree): Strict schema w/ closed handler enum, pure monotone session→agent→server engine, 3 builtins reading the canonical usage layer, flag-off `AGENT_SESSION_POLICIES_ENABLED` hook (DENY→durable event+`PolicyRefusal`, ASK→existing approval wall), **floors machine-proven** (4 tests incl. all-permutations stricter-first + no-grant-verdict lock) — commit a824377 on `feat/agt14-session-policies`, host-verified (validate PASS, 29 policy + 34 regression tests, ruff clean), Fable review APPROVE. **Packet 2 IMPLEMENTED 2026-07-24** (Sol xhigh): tighten-only policy gate before agent board-change proposals (DENY→403 before `make_proposal`, ASK→existing human-token wall, flag-off) + finding-4 fix; safety invariant machine-proven, `board_change.py` untouched (commits a7aa2d3, c32b6b1). **Fresh-Sol pre-enablement DEEP REVIEW verdict: DO-NOT-ENABLE** — 4 blockers tracked as **AGT-19** (observational-not-preventative enforcement, no mandatory server floor, cost fails open, ASK not human-verified). Flag stays OFF. Evidence: RUNDOC §9. **Awaiting operator merge (flag-off); enablement gated on AGT-19.**
...
**Notes:** Turn the hand-known capability matrix of the agent-session adapters (claude_agent, claude_code_local, codex_agent, openrouter_agent — Copilot when AGT-4 lands) into an executable conformance suite: each capability cell (streaming, resume, write-mode wall, attachments, model switch) is earned by a probe and reconciled against the adapter's declared profile; declared ✓ but observed ✗ = DRIFT failure. Per-adapter facts live on a self-declared profile object, never in probe code; offline/live probe split so CI runs the cheap subset. AGT-3-for-runtimes; drift count feeds AGT-12's KPI leaderboard. Run-doc: [docs/projects/agt-15-adapter-capability-bench/RUNDOC.md](../projects/agt-15-adapter-capability-bench/RUNDOC.md). **Packet 1 IMPLEMENTED 2026-07-24** (Sol gpt-5.6-sol high, isolated worktree): `cc adapter-bench` + `bench/` package, 5-dim declared-vs-observed matrix to `generated/`, per-adapter `BenchProfile`. Honesty rule enforced as a test (real adapters SKIPPED offline, never faked PASS; offline SKIPPED=22 DRIFT=0); DRIFT mechanism proven + coverage guard (every harness needs one profile). Report-only until a live baseline. Commit a2f9d5e on `feat/agt15-adapter-bench`, host-verified (validate PASS, 7 bench + 55 regression tests, ruff+mypy clean), Fable review APPROVE. **Packet 2 IMPLEMENTED 2026-07-24** (Sol high): added `interrupt` + `steering` dims (5→7); interrupt event-observable, steering honestly SKIPPED for FakeHarness (commit a2d2f10); 9 bench + 36 regression tests, ruff clean, Fable APPROVE. CI DRIFT-gate flip still gated on an operator live baseline. Evidence: RUNDOC §9. **Awaiting operator merge.**
