...
**Notes:** main's `contracts` workflow red since 2026-07-17; 12 failures in 4 families (capture-convert container-path reliance ×5, usage fixture time-drift, pickers stdin-signature drift, session-service durable user_message drift ×5). **FIXED 2026-07-23 — PR #78** (branch `feat/kan-25-ci-green` @ ed2be06): all test-side root-cause fixes, zero production changes; Codex gpt-5.6-sol (xhigh) implemented + failed closed on its sandbox; reviewer verified on host (48 family tests + regression guard exit 0, ruff clean); Fable review APPROVED. Run-doc: [docs/projects/kan-25-ci-green/RUNDOC.md](../projects/kan-25-ci-green/RUNDOC.md). Tail: PR #78's own CI = the live proof; operator merges. Also: delete the stray `C:\app` directory (artifact of the Windows-masked path bug, predates this fix).
