...
> **Archived — point-in-time review.** The durable decisions here (one control
> plane, desktop-rights-first ordering, don't import example KPIs as gates)
> still hold. The coverage map and "ordered next work" are a 2026-06-16
> snapshot — later work (desktop no-op canary, timeout/takeover policy,
> autonomous edits enabling) has since landed; check `docs/MASTER.md` §14
> for current status.
