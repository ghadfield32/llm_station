...
> **Archived — dated candidate evaluation.** Individual per-candidate verdicts
> are accurate as historical snapshots. Its own "Ordered work left" list has
> partially executed since (e.g. item 1, config-derived routing in
> `services/judge_gate/app.py`, is done) — do not treat that list as current
> status without checking code first.

This is the read-only evaluation pass for the broad AI-agent idea batch in
[agent-ideas-evaluation-prompt.md](../evaluation/agent-ideas-evaluation-prompt.md), focused
...
Sources: local [knowledge-format.md](../architecture/knowledge-format.md), [MASTER.md](../MASTER.md),
Agno source above.
...
Sources: local [improvement-loop.md](../improvement/improvement-loop.md),
[independent-verification.md](../improvement/independent-verification.md), and existing
