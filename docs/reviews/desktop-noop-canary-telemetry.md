...
> **Archived — restated in `docs/MASTER.md` §14 changelog (2026-06-20 entry)
> almost verbatim.** Kept here because the evidence-artifact paths and safety
> boundary are still the detailed reference; check `configs/autonomy.yaml`
> (`desktop_targets`) for current enablement state rather than trusting the
> prose below to stay current.
