...
> **Archived — superseded, do not follow the recommendation below.** This
> doc's central recommendation ("take no urgent action" on cross-conversation
> memory) was overtaken the very next day: durable cross-conversation memory
> was built and live-validated 8/8 on 2026-06-13/06-14. See `docs/MASTER.md`
> §14 changelog and [memory-integration-patch.md](memory-integration-patch.md)
> for what actually shipped. Kept only as a historical record of the
> within-conversation-recall proof, which is still accurate.
