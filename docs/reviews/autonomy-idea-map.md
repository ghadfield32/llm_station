...
> **Archived — superseded and stale.** The tool count ("15 tools") and bridge
> path (`scripts/kanban_bridge.py`) below are wrong (see
> `docs/kanban/kanban-integration.md` for the fix); the diagram's VPS-centric
> framing and Hermes-as-core-orchestrator claim are also outdated. Current
> architecture lives in `docs/MASTER.md` §4 and the diagrams in
...
| Chat gateways (Discord/Slack/Telegram/WhatsApp) | `src/command_center/channels/` (LiteLLM `triage`; see [channels.md](../architecture/channels.md)) | built; each needs its token |
