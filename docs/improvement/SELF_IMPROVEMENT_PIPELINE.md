...
`docs/reference/betts-basketball-standards/PIPELINE_STANDARDS_TEMPLATE.md` §0.11a ("add the stage registry row first").
...
|---------|--------|------|:----:|
| First-party Self Improvement Kanban | `kanban.py` + `board_feed.py` | top findings → deterministic Backlog cards; all registered opportunity boards are scanned automatically, while the output board is excluded to prevent recursion. Human approval remains outside the DAG. | ✅ |
