...
- In live mode it can upsert the brief into the AppFlowy `review` DB; in
...
def due_lessons(lessons: list[Lesson], on: date | None = None) -> list[Lesson]:
    on = on or date.today()
    return [l for l in lessons if l.next_review and l.next_review <= on]
...
def render_brief(new_papers, new_repos, new_signals, due, next_book) -> str:
    ...
    out += section("Review today", due, lambda l: f"- {l.lesson}  _( {l.domain} )_")
    return "\n".join(out)
...
# AppFlowy, render the brief, and upsert it into the `review` database (one
...
def main():
    ...
    af = AppFlowyClient(st.appflowy_base_url, st.appflowy_workspace_id,
                        dry_run=st.growthos_dry_run)
    if st.growthos_dry_run:
        log.info("dry-run: brief needs live AppFlowy reads; writing file only")
        md = render_brief([], [], [], [], "")
        print(write_brief_file(md))
        return

    af.login(st.appflowy_email, st.appflowy_password)
    papers = _recent(af, "papers", "Title")
    repos = _recent(af, "repos", "Name")
    signals = _recent(af, "signals", "Headline")
    due = _due_lessons(af)
    md = render_brief(papers, repos, signals, due, _next_book(af))
    ...
    wrote = af.upsert("review", [{"pre_hash": f"brief-{today}",
