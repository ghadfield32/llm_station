...
BoardBackend = Literal["appflowy", "local"]
...
def publish_suggestions(
    *,
    ...
    suggestions = [
        item
        for item in _load_suggestions(base)
    ...
    if backend == "appflowy":
        ...
        try:
            cards = client.list_cards()
            result = _publish_into_cards(cards, suggestions, apply=apply)
            if apply:
    ...
    state = load_local_state(base, config)
    result = _publish_into_cards(state["cards"], suggestions, apply=apply)
    if apply:
...
def process_selected(
    *,
    ...
    if backend == "appflowy":
        ...
        try:
            cards = client.list_cards()
            result = _process_selected_cards(cards, base, apply=apply, executor=executor)
            if apply:
                for card in result["cards_to_write"]:
                    client.upsert_card(card)
        finally:
    ...
    state = load_local_state(base, config)
    result = _process_selected_cards(state["cards"], base, apply=apply, executor=executor, state=state)
    if apply:
        state["cards"] = result["cards"]
        save_local_state(base, state, config)
    result.pop("cards_to_write", None)
...
def _process_selected_cards(
    ...
    *,
    ...
    working = json.loads(json.dumps(cards))
    selected = [card for card in working if card.get("column") == "Selected by Geoff"]
    ignored = [
        {"card_id": card.get("card_id"), "column": card.get("column")}
        for card in working
        if card.get("column") != "Selected by Geoff"
    ]
