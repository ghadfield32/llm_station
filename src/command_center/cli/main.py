...
COMMANDS: dict[str, tuple] = {
    ...
    "assistant-doctor": (lambda a: pym("command_center.cli.assistant_doctor", *a),
                         "does the running agent worker match this checkout? (drift check)"),
    "assistant-verify": (lambda a: pym("command_center.cli.assistant_verify", *a),
                         "run per-assistant evidence suites; feed the ranking leaderboard"),
    "mission-run": (lambda a: pym("command_center.cli.mission_run", *a),
                    "governed mission execution self-check (flag-gated, local draft-PR only)"),
    ...
    "model-canary": (lambda a: pym("command_center.cli.model_ops", "canary", *a),
                     "render a local canary for a role (--apply writes generated config)"),
    "model-promote": (lambda a: pym("command_center.cli.model_ops", "promote", *a),
    ...
    "backup": (lambda a: pym("command_center.cli.runtime_backup", *a),
               "immutable runtime snapshots: create|verify|list|restore-to-staging"),
    ...
    "grand-todo-import": (lambda a: pym("command_center.cli.grand_todo_import", *a),
                          "merge Betts GRAND TODO into its governed board (dry-run; --apply)"),
    "appflowy-migrate": (lambda a: pym("command_center.cli.appflowy_migrate", *a),
                         "audited AppFlowy history -> first-party boards (dry-run; --apply)"),
    "book-enrich": (lambda a: pym("command_center.cli.book_enrich", *a),
                    "source-backed Books metadata enrichment (dry-run; --apply)"),
    ...
    "content-find": (lambda a: pym("command_center.cli.reference", "find", *a),
                     "find a post/board/doc/library by meaning (alias+fuzzy+semantic), not exact name"),
