...
## AGT-14/15/16 PACKET 2 + pre-enablement deep review = DO-NOT-ENABLE (2026-07-24)
- **AGT-15 p2** (Sol high, a2d2f10): interrupt + steering bench dims (5→7); interrupt event-observable, steering honestly SKIPPED for FakeHarness. 9 bench + 36 regression, ruff clean, Fable APPROVE.
- **AGT-16 p2** (Sol high, 0f49557): read-only `GET /api/agent-session-specs` (redacts instructions/secrets, malformed→typed error not 500) + runtime-agnostic chrome spec picker (display-only, DESIGN.md-conformant) + AGT-10 emission seam. validate + endpoint + web tests + tsc + **vite build** all green (build run host-side; sandbox blocks esbuild). Fable APPROVE.
- **AGT-14 p2** (Sol xhigh, a7aa2d3 + finding-4 fix c32b6b1): tighten-only policy gate BEFORE agent board-change proposals — DENY→403 before `make_proposal`, ASK→existing human-token wall (no 2nd surface), flag-off. Safety invariant machine-proven (no verdict applies without the human token; `board_change.py` untouched). Flag OFF.
- **FRESH-SOL PRE-ENABLEMENT DEEP REVIEW (read-only, xhigh) → VERDICT DO-NOT-ENABLE.** The gate did its job. What HELD: board wall never bypassed, `resolve()` monotone, closed enum (no import injection), no permission-profile mutation, flag-off identity (after finding-4). **4 blockers → AGT-19 (BLOCKED, P1)**: (1) CRITICAL enforcement is observational not preventative (evaluates after the harness began; ASK auto-passes; adapters record approval `effective:False`), (2) no mandatory server floor (unspec'd session→ALLOW), (3) cost fails open silently, (4) ASK not human-verified. **Flag stays OFF; enablement gated on AGT-19 → re-review.** All packets land flag-off = zero main behavior change. Branch feat/agt-packet2-followons → PR to main.
